...
| **Bypass/enable block** | `/BlockEnableSet` | `,iiii` | `[cmd, dsp, block_id, enable]` — e.g. `[115,0,3,0]` bypass, `[116,0,3,1]` on | echoed as `/setBlockEnable [devId, seq, dsp, block, enable]` on 2001. **`block_id` = the `sfg_.flow[dsp].blks` position key.** [ERRATUM 2026-07-14, loudness phase 0: **wrong** — `block_id` is the **wire index `(blks_key−1)/2`** (the captured `3` = blks key 7, the amp). At a raw blks key the device echoes success but toggles the wrong block, and `/ParamValueSet` silently drops the write — the echo-based confirmation below was a false positive. See `2026-07-14-loudness-feedback-normalization.md` §phase-0 findings.] The toggle is **volatile** — audible immediately but NOT written to the content blob's `enbl` (base enable) until the preset is saved; so `/EditBufferStateGet` read-back does not reflect a live bypass. helixgen confirms the write via the 2001 echo, not a read-back. |
