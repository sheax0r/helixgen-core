...
- `helixgen device blocks [--json]` — list the **live edit buffer's blocks** with their `(path, block)` coordinates, model name, and saved base on/off. Read-only. These are the coordinates `device bypass`/`device model`/`device set-param` address.
...
- `helixgen device bypass <path> <block> <on|off>` — **bypass/enable a block** in the live edit buffer (`/BlockEnableSet [dsp, wire_block, enable]`; the `device blocks` key is translated to the wire's `(key-1)/2` internally — HW-verified 2026-07-14 by meter response; the echo alone is NOT proof, the device happily echoes a toggle of the wrong block). The toggle is *volatile* (audible at once, not written to the preset until you save, so `device blocks` won't reflect it).
