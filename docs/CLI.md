...
- `helixgen add-block <preset> <block> [--path N] [--after NAME]` — insert a block (append to `--path`, default 0, or after a named block). The new block arrives with every param at its **model default** (hgc-x7i) — the same baseline `generate` uses — not at the block library's `sighted` value.
