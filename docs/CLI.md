...
- `helixgen add-block <preset> <block> [--path N] [--after NAME]` — insert a block (append to `--path`, default 0, or after a named block). The new block arrives with every param at its **model default** (hgc-x7i) — the same baseline `generate` uses — not at the block library's `sighted` value. **The row's existing blocks keep their grid slots** (hgc-hhp): the new block goes next to its predecessor and only the unbroken run of blocks in the way slides one slot into the nearest empty one — either direction, the shorter move wins, ties go right — so an insert into a gapped row uses the gap instead of moving the whole row, which is what the hardware does. Sliding LEFT is what saves the boxed-in rows: `[1,2,3,4,5,6,12]` (factory 2A) has nowhere to go on the right for an append, and moving the one block on `b12` down to `b11` beats re-packing all seven. Only a run boxed in on BOTH sides falls back to re-packing the lane onto `b01..bn` as it always did — reported on stderr and in the `patch --json` warnings list — rather than refusing an edit that used to work. A lane that would need a 13th slot still errors.
- `helixgen remove-block <preset> <block> [--path/--lane/--pos]` — delete a block. **The freed slot stays a hole and nothing else moves** (hgc-hhp) — the row is not re-packed, matching both the hardware and what the preset recorded. A MIDI CC binding on the removed block is dropped with a warning; every other binding keeps its coordinate.
- `helixgen swap-model <preset> <old> <new> [--path/--lane/--pos]` — replace a block with another of the **same category**; carries over params the target shares, warns on any it has to drop (surface those warnings to the user). Params the target has but the old block did not start at the target's **model default** (hgc-x7i), not at its `sighted` value.
- `helixgen view <preset.hsp> [-o recipe.json]` — read-only projection of a `.hsp` back into the recipe shape (replaces the old `decompile`; the `-o` dump is non-authoritative). Prints JSON by default. Params are listed only where they differ from the **model default** — the same baseline `generate` fills an unmentioned param from, rounded to the device's float32 precision so a real export's `0.7070000171661377` matches the asset's `0.707` — so the projection round-trips, and an absent param means "at the factory position". The converse is *nearly* true: a handful of params whose exporter wrote a truncated decimal (`0.00999999`) still list as overrides.
...
- `helixgen device to-hsp <file.sbe|cid> -o <out.hsp> [--name N] [--author A] [--library D] [--no-verify]` — **the reverse transcoder: device content (`_sbepgsm`) → a helixgen `.hsp`.** The inverse of `install`, so a preset authored **on the hardware or in HX Edit** stops being an opaque blob and becomes a real `.hsp` that `view`, `patch` and the surgical edit verbs all handle and that `install`/`sync` push straight back. SOURCE is either a local `.sbe` path (**wholly offline** — no device, no lease, no dangling-token check) or a device **CID** (an integer; read via the non-activating `/GetContentData`, so the live tone is never disturbed, and the preset's device name becomes `meta.name` — the content blob itself carries no name). Fidelity covers models, params, snapshots (per-scene bypass + param deltas), footswitch/EXP assignments, IR references, base bypass, input routing + impedance, output level/pan, scribble strips, **every model slot of a dual-cab block** (the B cab's own mic/level/EQ — bead hgc-q38), and the full signal graph (dual-DSP, parallel splits, dual-amp). Every mapping is the inverse of a specific piece of `device/transcode.py` + `device/bridge.py` — which is what makes `.sbe → .hsp → .sbe` a free oracle. Design + the full residual-diff analysis: [`docs/superpowers/specs/2026-08-12-sbe-to-hsp-reverse-transcoder-design.md`](superpowers/specs/2026-08-12-sbe-to-hsp-reverse-transcoder-design.md).
  ...
  - **`--verify`** (default on) re-transcodes the `.hsp` and reports whether it reproduced the source bytes. Content **helixgen itself installed** comes back **byte-exact** (61 of the 66 real device blobs in the development corpus). Content the **device re-saved** differs in device-side serialization conventions only — `pm__` key order, the `snps` array's `si__` ordering, msgpack map-key order inside `ctrl` records, `cg__` target/source/controller id numbering (plus the block `id__`/`bmap` permutation a hardware reorder leaves behind), and the `hist`/`selb`/`self` edit-buffer scratch keys. None of that is tone: the conversion is a **canonicalization** that reaches a **fixed point after one pass** — **all 66** of Line 6's factory presets are a fixed point, with no residual `.hsp` diff of any kind. What it took, beyond the grid positions below: a snapshot target survives even when its 8 values are equal (hgc-xh3), a flow's ENDPOINTS are re-emitted as recorded rather than as fixed templates (`P35_OutputPath2A` at b13, a real row-1 input/output pair — hgc-ikp), and snapshot/controller targets are spelled on a dual-cab's B model slot (hgc-3yc), on the endpoints themselves (hgc-5us — including a row-1 `InputNone`/`OutputNone` the device snapshot-tracks, which is emitted into the `.hsp` only when it carries such state, since the forward path re-synthesizes a bare one: hgc-6av) and on split/join routing nodes (hgc-rq3). Block **grid positions come back verbatim, gaps included** (bead hgc-8o6) — a row whose blocks sit on non-contiguous slots, which 61 of Line 6's 66 factory presets do, re-installs onto the same slots; only a coordinate that cannot be honoured (off-grid, two blocks on one slot, a split/join recorded in row 1) falls back to a contiguous row, rather than emitting an invalid grid. **Surgical edits hold that layout too** (bead hgc-hhp): `remove-block` leaves the freed slot as a hole and `add-block` slides only the unbroken run of blocks in the way one slot into the nearest gap — see their entries above for the boxed-in case that still re-packs. `tests/test_untranscode.py` pins both the fixed point and an **id-agnostic semantic-equivalence projection** (blocks by grid slot, params by pid, snapshot scenes and controller assignments resolved through the target graph) across the whole corpus.
  - **NOT yet reversed:** Command Center commands (#16) and MIDI CC controller bindings (#33) are **dropped, with a warning naming how many**. Both are EXPERIMENTAL in the forward direction and neither appears in the validation corpus. Per-block `harness` state is emitted only for the input/output endpoints (matching a real export); a user block's `hrns` params and its `hrns.enbl` are not carried, because the forward path ignores `.hsp` `harness` entirely and synthesizes a canonical `hrns` — so round-trip fidelity is unaffected either way. **Everything else that cannot be carried is reported on stderr, one line per loss** (a disabled DSP path, row-1 blocks with neither a split nor a row-1 input feeding them — they keep their slots, but a re-install writes `InputNone`/`OutputNone` into row 1 so nothing reaches them — a controller source with no `.hsp` id, a snapshot target the device did not write into every VALID snapshot's `tamv`). Silence means nothing was dropped. An **unused** snapshot carries `vald: False` and an EMPTY `tamv` — that is the device's own shape, not a loss, so its slots are filled with the target's base value on the way in and left empty on the way out (bead hgc-oqd; requiring a value in all 8 used to drop every scene on 16 of the 66 factory presets, and the `.sbe` oracle could not see it because the loss was symmetric).
  - **Not every stderr line is a loss.** Device state the rest of helixgen does not model is reported there too. One case exists today (bead hgc-0d7): the device stores the value a knob was LIVE at when the preset was saved (`parm.valu`) beside the snapshot scenes (`tamv`), and the two need not agree — it applies the plain value on load and the snapshot's only once you SELECT a snapshot. Both halves are carried verbatim rather than normalized (normalizing would change what the preset sounds like the moment it loads), and the divergence is reported, because a later base-level `set-param`/`enable` re-syncs the other half. 0 of the 66 factory presets carry it; 2 of 66 real device backups do.
