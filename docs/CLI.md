...
- `helixgen add-block <preset> <block> [--path N] [--after NAME]` — insert a block (append to `--path`, default 0, or after a named block). The new block arrives with every param at its **model default** (hgc-x7i) — the same baseline `generate` uses — not at the block library's `sighted` value. **The row's existing blocks keep their grid slots** (hgc-hhp): the new block takes the slot right after its predecessor, and only the unbroken run already sitting there slides one slot right, stopping at the first empty slot — so an insert into a gapped row uses the gap instead of moving the whole row, which is what the hardware does. The one exception is an insertion point with no empty slot to its right (a block parked on `b12`): the lane then re-packs onto `b01..bn` as it always did, warning on stderr, rather than refusing an edit that used to work. A lane that would need a 13th slot still errors.
