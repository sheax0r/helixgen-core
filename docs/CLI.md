...
- `helixgen library show <name> [--json]` — one tone's — or one guitar
  ...
  presence, each variant's key/preset_name/hsp path, plus the variant's
  `normalized` record when `device normalize --yes` has written one — date,
  non-zero-trim count or "in band", scope); if no tone matches it is
...
- `helixgen describe <tone>` — human-oriented write-up: header ("Artist -
  ...
  guitar_settings, and a brief `normalized` line when `device normalize
  --yes` has recorded one), then the full `description_md` verbatim below a
