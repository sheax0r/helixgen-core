...
- `helixgen device save <name> --setlist <name> --pos <N>` — save the live edit buffer as a new preset (slot must be empty; the emptiness check is strict — backlog #40 — a listing timeout aborts the save rather than reading the slot as empty).
- `helixgen device rename <cid> <new_name>` — rename a preset.
- `helixgen device delete <cid> [--setlist <name>] [--yes]` — delete a preset.
- `helixgen device set-param <path> <block> <param_id> <value>` — set one edit-buffer param (`/ParamValueSet`). `<block>` is the `device blocks` coordinate (translated to the wire's `(key-1)/2` internally — HW-verified 2026-07-14; before that fix this verb always failed since the device silently drops writes at raw keys). `<value>` is in the param's **raw units** (e.g. dB for the output block's `gain` pid 2), not normalized.
...
- `helixgen device push <file.sbe> <name> --pos <N>` — install a local content file into a new slot (restore/clone; the slot-emptiness check is strict — backlog #40 — a listing timeout aborts rather than reading the slot as empty).
