...
- `helixgen device sync [<dir>] [--setlist user|factory|throwaway] [--exclude-irs] [--template <cid>]` — **bulk-sync a whole directory of authored `.hsp` tones onto the device.** There is no single "tone library" file — authored tones are loose `.hsp` files, so `sync` globs `<dir>` (default: the `preset_output_dir` preference) and installs each into an **empty** slot of the setlist (non-destructive fill-empty; never overwrites), uploading each tone's referenced IRs (unless `--exclude-irs`) and recording every placement in the slot ledger. **Idempotent**: a tone already on the device (matched by name) is skipped, so re-running only adds what's new. Per-tone install/IR failures are reported without aborting the run. (Install still needs a `--template` whose block categories cover the tones — see `install` above.) EXPERIMENTAL.
