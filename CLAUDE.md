...
- `helixgen device push-ir <file.wav>` — upload an impulse response to the device over SFTP (the device auto-registers it; registration is delayed without the editor running, and the IR listing lags — confirm via `list-irs` / `get_ref`). EXPERIMENTAL.
- `helixgen device pull-ir <filename> <outfile>` — download an IR `.wav` by its on-device filename. EXPERIMENTAL.
- `helixgen device install <preset.hsp> <name> --pos <N> [--template <cid>] [--auto-irs]` — **author a helixgen `.hsp` onto the device as a new, playable preset** (the `/tone` → on-your-amp path). Maps the preset's blocks onto a device template's same-category slots (v2.2: single serial chain; `--template` picks the skeleton preset, default = current edit buffer). Model/param names bridge helixgen↔device via `device/modelmap.py` + `device/defs.py`; writes are byte-faithful via `/SetContentData`. `--auto-irs` uploads any IRs the preset references that aren't already on the device (resolving each `irhash` to a local WAV via `mapping.json`, then `push-ir`), and verifies the device registered each under the expected hash so the cab resolves. This is reliable as of the atomic-upload fix — the earlier "helixgen `irhash` disagrees with the device" symptom was a non-atomic-upload race (the device hashed a half-written file), now fixed by staging + rename in `device/sftp.py` (see `docs/helix-sftp-access.md`). EXPERIMENTAL.
