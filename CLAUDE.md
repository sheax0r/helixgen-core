...
- IR registration (`register-irs`, `ir-scan`) **copies WAVs into `library/irs/<pack>/` with metadata sidecars** by default (`--no-copy` opts out). Direct hashing needs libsndfile + **48 kHz sources** — helixgen input constraint, not device's (device normalizes any rate on own import, so non-48k IR still works once imported onto hardware). A **wedged** IR (backing file resolves, absent from device registry) reads as present, so auto-upload paths (`install --auto-irs`, `device sync`) skip it and the cab is silent with no error — watch stderr for the cross-check warning; clear with `device delete-ir --force-wedge`, or re-run `device push-ir` on the same WAV — it tells a stale listing cache from a wedge (nudged-listing check) and heals the wedge by re-import, but only when the listing has a row to nudge: an empty or failed listing (the single-wedged-IR case) keeps the trusting "already" answer, so `--force-wedge` stays the sure clear (#93; the auto-upload skip path remains open too).
