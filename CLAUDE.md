...
- `src/helixgen/device/` — network device control (OSC-over-ZeroMQ client, `transcode` = `.hsp` -> device content, `untranscode` = device content -> `.hsp`, `modelmap`, `defs`, setlist manifest)
