...
add-guitar`, `library import` (bring external `.hsp` under naming schema; never overwrites), `library fork` (below), `library migrate` (one-shot idempotent pre-library-to-library migration; `--dry-run` first), `library ir-backfill` (copy + scaffold metadata for IRs registered before library layout).

**Forking is a verb, never a re-author** (hgc-2ja). "Make me an EC-1000 version" = `helixgen library fork <source> --guitar <g>`: it COPIES the source variant's `.hsp` (only `meta.name` rewritten), so blocks/params/snapshots/footswitches/IRs come across verbatim. **A tone's write-up is never read for values** — rebuilding a fork from `description_md` is exactly how pre-rewrite settings come back, and the source write-up lands in the fork's `notes_md` behind an INHERITED banner precisely so nobody mistakes it for a description of the fork. `--guitar` alone = second **variant** of the same logical tone; `--artist`/`--song`/`--descriptor` = new logical tone. Provenance goes in the variant's `forked_from` (readable via `describe` / `library show --json`). helixgen **never re-voices**: differing pickup classes only print an advisory stderr checklist naming the drive/brightness/input/level params THIS chain has. Two identity guards ship with it — an explicit `--descriptor` carrying a smuggled identity (`" - "` separator, trailing guitar name) is refused on `generate`/`library import`/`library fork`, and `generate -o` combined with naming flags warns instead of discarding them silently.
