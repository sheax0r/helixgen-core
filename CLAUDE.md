...
- `helixgen device push-ir <file.wav>` — import an impulse response onto the device. Uploads the **device-canonical processed IR** (`helixgen.ir.write_stadium_ir` — the same 8192-sample file the editor uploads, whose data-chunk MD5 is the `irhash`), not the raw WAV, so the device registers it under exactly the hash a preset references. Registration itself is **device-gated**: the editor's own import registers instantly, but an external SFTP write registers only on the device's slower scan or the next editor import (the hash is correct whenever it lands). Confirm with `list-irs` / `get_ref` / `ir_path_for_hash`. EXPERIMENTAL.
